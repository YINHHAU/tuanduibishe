  ...
  <el-menu class="side el-menu-vertical-demo" default-active="/" @open="handleOpen" @close="handleClose"
    ...
    <el-submenu index="user"
      ...
      <el-menu-item index="/user/table" v-show="user_group == '管理员'"
        @click="handleButtonClick(8)"><span>管理员</span></el-menu-item>
      <el-menu-item index="/employee_users/table" v-show="user_group == '管理员' || $check_action('/employee_users/table')"
        @click="handleButtonClick(8)">
        <span>员工用户</span></el-menu-item>
      <!-- <el-menu-item index="/user_group/table"><span>用户组</span></el-menu-item> -->
    </el-submenu>


    <el-submenu class="Cust" :index="index" v-for="(i, index) in auth" :key="i.auth_id"
      ...
      <el-menu-item
        v-if="i.has_add && (user_group == '管理员' || $check_action(i.path.replace(/(.*)table/, '$1view'), 'add'))"
        :index="i.path.replace(/(.*)table/, '$1view')" :class="{ sactive: $route.path === i.path }"
    ...
    <el-submenu class="Cust">
      <template slot="title"><i class="el-icon-news"></i>库存预警管理</template>
      <el-menu-item @click="clickList">预警列表</el-menu-item>
      <el-menu-item @click="clickDetail">预警详情</el-menu-item>
    </el-submenu>
...
export default {
  ...
  methods: {
    clickList(){
      this.$router.push('/stock_warning/table')
    },
    clickDetail(){
      this.$router.push('/stock_warning/view') 
    },
  ...
  computed: {
    auth() {
      ...
      site_list.push("outbound_information")
